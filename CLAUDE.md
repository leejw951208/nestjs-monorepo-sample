...
This is a NestJS monorepo containing two applications (user-app and admin-app) with shared libraries. The project uses Prisma ORM with PostgreSQL, JWT authentication with Redis-based token management, Winston logging, and Swagger API documentation.
...
**Applications:**
- `apps/user-app/` - User-facing API (port 3000)
- `apps/admin-app/` - Admin API (port 3001 in Docker)
...
3. **cookieParser** - Parse cookies (user-app only)

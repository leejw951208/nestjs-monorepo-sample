...
export class PostController {
    ...
    @Post()
    async createPost(@JwtPayload() payload: JwtPayloadType, @Body() reqDto: PostCreateDto): Promise<PostResDto> {
    ...
    @ApiOkResponse({ type: OffsetPageResDto<PostResDto> })
    @Get('me')
    async getMyPosts(@JwtPayload() payload: JwtPayloadType, @Query() query: PostListReqDto): Promise<OffsetPageResDto<PostResDto>> {
    ...
    @ApiOkResponse()
    @Delete(':id')
    async deletePost(@JwtPayload() payload: JwtPayloadType, @Param('id', ParseIntPipe) postId: number): Promise<void> {

...
| 메서드 | 경로 | 설명 | 인증 | 상태 코드 |
|--------|------|------|------|-----------|
| POST | `/signup` | 회원가입 | 불필요 | 201 |
| POST | `/signin` | 로그인 | 불필요 | 200 |
| DELETE | `/signout` | 로그아웃 | 필요 | 204 |
| POST | `/token/refresh` | 토큰 재발급 | Refresh Token | 200 |
| POST | `/password-reset/request` | 비밀번호 재설정 코드 발급 | 불필요 | 201 |
| POST | `/password-reset/verify` | 코드 검증 + 리셋 토큰 발급 | 불필요 | 200 |
| PATCH | `/password-reset` | 비밀번호 변경 | 불필요 | 204 |
...
| GET | `/posts/me/offset` | 내 게시글 (Offset) | - | 200 |
| GET | `/posts/offset` | 전체 게시글 (Offset) | - | 200 |
| GET | `/posts/me/cursor` | 내 게시글 (Cursor) | - | 200 |
...
| 메서드 | 경로 | 설명 | 상태 코드 |
|--------|------|------|-----------|
| GET | `/notifications/me/cursor` | 내 알림 목록 (Cursor) | 200 |
| GET | `/notifications/:id` | 알림 상세 조회 | 200 |
| PATCH | `/notifications/:id` | 알림 읽음 처리 | 204 |
| PATCH | `/notifications/read-all` | 전체 알림 읽음 처리 | 204 |
...
| 메서드 | 경로 | 설명 | 인증 | 상태 코드 |
|--------|------|------|------|-----------|
| POST | `/signin` | 관리자 로그인 (loginId) | 불필요 | 200 |
| DELETE | `/signout` | 관리자 로그아웃 | 필요 | 204 |
| POST | `/token/refresh` | 토큰 재발급 | Refresh Token | 200 |
...
| PATCH | `/users/:id/status` | 사용자 상태 변경 | `user:update` | 204 |
...
| PATCH | `/posts/:id/status` | 게시글 상태 변경 (숨김/발행) | `post:update` | 204 |
| DELETE | `/posts/:id` | 게시글 삭제 | `post:delete` | 204 |
...
| AUTH_ERROR_009 | 400 | 인증 코드가 일치하지 않습니다 |
| AUTH_ERROR_010 | 400 | 인증 코드가 만료되었습니다 |
| AUTH_ERROR_013 | 400 | 유효하지 않은 비밀번호 재설정 토큰입니다 |
| AUTH_ERROR_014 | 429 | 인증 코드 입력 횟수를 초과했습니다 |
